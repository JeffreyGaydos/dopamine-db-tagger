...
function Landing_AddHandlers() {
    ...
    document.querySelectorAll("form #sequence-type, form #sequence-order").forEach(el => {
...
function SubmitSearch() {
    ...
    fetch(`http://localhost:8080/api/search/tracks/${encodeURIComponent(queryString)}`).then((f) => {
        f.json().then(r => {
            ...
            const sequenceType = document.querySelector("#sequence-type").value;
            const ascending = document.querySelector("#sequence-order").checked;
            r.forEach(sr => {
                const link = document.createElement("A");
                link.innerText = `${sr.TitleRaw}`;
                link.href = `http://localhost:8080/ui/tagging/${sr.TrackID}?seq=${sequenceType}&dir=${ascending}`;
